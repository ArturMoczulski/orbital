...
> {
  ...
  /**
   * Save a domain object with document attached
   * @param domainObject The domain object with document attached
   * @returns The saved domain object with document attached
   */
  async save(domainObject: WithDocument<T, S>): Promise<WithDocument<T, S>> {
    if (!domainObject.document) {
      // If no document is attached, create or get one
      if (domainObject._id) {
        // Try to find existing document
        const existingDoc = await this.model.findById(domainObject._id).exec();
        if (existingDoc) {
          // Update existing document with current domain data
          const data = PersistenceMapper.toPersistence(domainObject);
          Object.assign(existingDoc, data);
          return DocumentHelpers.attachDocument(
            domainObject,
    ...
    // Update the document with current domain data
    const data = PersistenceMapper.toPersistence(domainObject);
    Object.assign(domainObject.document, data);

    // Save the document
    await domainObject.document.save();
  ...
  /**
   * Execute a query and convert results to domain objects with documents
