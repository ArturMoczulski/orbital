...
describe("parseWithReferences with circular references", () => {
  ...
  it("should handle direct circular references without stack overflow", () => {
    ...
    // With the current implementation, we get multiple issues for the array
    expect(result.error?.issues.length).toBe(4);
    // But we should still have the specific issue we're looking for
    const nonexistentIssue = result.error?.issues.find(
      (issue) => issue.path.join(".") === "friendIds.2"
    );
  ...
  it("should handle nested circular references without stack overflow", () => {
    ...
    const result = parseWithReferences(OrganizationSchema, organization, {
      ...
      maxDepth: 2, // Increase maxDepth to validate nested references
    });

    // With maxDepth=1, we should only validate the immediate references
    expect(result.success).toBe(false);
    // The current implementation reports issues for all fields in nested objects
    expect(result.error?.issues.length).toBe(12);

    // Check that the issues are detected
    const paths =
      result.error?.issues.map((issue) => issue.path.join(".")) || [];
    // We should have at least one of these issues
    const hasManagerIdIssue = paths.some((path) => path.includes("managerId"));
    const hasDepartmentIdIssue = paths.some((path) =>
      path.includes("departmentId")
    );
    expect(hasDepartmentIdIssue).toBe(true);
  });
  ...
  it("should handle deeply nested circular references", () => {
    ...
    // With maxDepth=1, we'll get validation errors for all fields
    // since we're using maxDepth=2 in the previous test, let's be consistent
    const result2 = parseWithReferences(
      EmployeeWithProjectsSchema,
      complexData,
      {
        dependencies: {
    ...
    // With maxDepth=2, we should detect the invalid reference
  ...
  it("should handle maximum recursion depth", () => {
    ...
    // With maxDepth=3, we should validate deeper but not hit the limit
    // since we're only validating immediate references by default
    expect(result.success).toBe(true);

    // Now test with a very low max depth to trigger the limit
    const resultWithLowDepth = parseWithReferences(NodeSchema, deeplyNested, {
      dependencies: {
        node: testNodes,
      },
      maxDepth: 0, // Set to 0 to trigger the limit immediately for nested fields
    });

    // Should fail due to max depth
    expect(resultWithLowDepth.success).toBe(false);
    expect(resultWithLowDepth.error?.issues[0].message).toContain(
